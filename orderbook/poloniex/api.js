...
function Poloniex(options) {
  ...
  this.url = config.get('apis:poloniex') + '/public?command=returnOrderBook'
}
 ...
 */
Poloniex.prototype.pullOrderBook = function() {
  ...
  /*
   * call Poloniex api to get a snapshot of latest orderbook
   * preprocess data to return an object of relevant data
   * to be further formatted and saved to hbase
   * poloniex of a weird structure that puts counter before base
   * in its api call
   */
  function getSnapshot(object) {
    const pair = object.counter.toUpperCase() + '_' + object.base.toUpperCase()
    const url = object.url + '&currencyPair=' + pair
