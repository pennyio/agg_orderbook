...
function Kraken(options) {
  ...
  this.url = config.get('apis:kraken') + '/0/public/Depth'
}
 ...
 */
Kraken.prototype.pullOrderBook = function() {
  ...
  /*
   * call Kraken api to get a snapshot of latest orderbook
  ...
  function getSnapshot(object) {
    const pair = object.base.toUpperCase() + object.counter.toUpperCase()
    const url = object.url + '?pair=' + pair
    let counterCurrency = object.counter
    if (object.counter === 'XBT') {
      counterCurrency = 'BTC'
    }
    ...
    return request({
      ...
      const sortedBids = orderbook.result[accessCodes[object.counter]]
      ...
      const sortedAsks = orderbook.result[accessCodes[object.counter]]
      ...
      new_orderbook.counter = counterCurrency
