...
function Bitfinex(options) {
  ...
  this.url = config.get('apis:bitfinex') + '/v1/trades/'
}
 ...
 * return index
 */
Bitfinex.prototype.getLastTrade = function() {
 ...
 */
Bitfinex.prototype.pullTransactions = function() {
  const self = this
  ...
  /*
   * call Bitfinex api to get a snapshot of latest trades
   * object contains lastTrade and lastTrade contains index
   * base XRP counter BTC
   */
  function getSnapshot(last) {
    const pair = (self.base + self.counter).toUpperCase()
